...
namespace gradepenalty_reattemptmaxscore\table;
...
class penalty_rule_table extends sql_table {
    ...
    public function __construct($uniqueid, $contextid) {
        ...
        // Add columns header.
        $this->define_headers([
            get_string('reattemptby', 'gradepenalty_reattemptmaxscore'),
            get_string('penalty', 'gradepenalty_reattemptmaxscore'),
        ]);
    ...
    protected function get_sql($count = false): string {
        if ($count) {
            ...
            $select = "r.reattemptby, r.penalty, r.sortorder";
            $order = " order by r.sortorder ASC";
        }

        $sql = "SELECT $select
                  FROM {gradepenalty_reattemptmaxscore_rule} r
     ...
     */
    public function col_reattemptby($row): string {
        ...
        if (count($this->rawdata) === ($row->sortorder + 1)) {
            ...
            foreach ($this->rawdata as $rule) {
                if ($rule->sortorder == $row->sortorder - 1) {
                    return get_string('reattemptby_lastrow', 'gradepenalty_reattemptmaxscore', $rule->reattemptby+1);
                }
            }
            // There is only one row.
            return get_string('reattemptby_onerow', 'gradepenalty_reattemptmaxscore');
        }

        // This is not the last rule.
        return get_string('reattemptby_row', 'gradepenalty_reattemptmaxscore', $row->reattemptby);
    }
}
