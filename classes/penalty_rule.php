...
class penalty_rule extends persistent {
    ...
    const TABLE = 'gradepenalty_reattemptmaxscore_rule';
    ...
    /**
     * Validate the reattemptby before saving.
     *
     * @param int $value reattemptby value.
     * @return true|lang_string error message if reattemptby is invalid.
     */
    protected function validate_reattemptby($value): bool|lang_string {
        if ($value < constants::REATTEMPTBY_MIN) {
            return new lang_string('error_reattemptby_minvalue', 'gradepenalty_reattemptmaxscore', constants::REATTEMPTBY_MIN);
        }
        return true;
    }
    ...
    protected function validate_penalty($value): bool|lang_string {
        if ($value < constants::PENALTY_MIN) {
            return new lang_string('error_penalty_minvalue', 'gradepenalty_reattemptmaxscore', constants::PENALTY_MIN);
        } else if ($value > constants::PENALTY_MAX) {
            return new lang_string('error_penalty_maxvalue', 'gradepenalty_reattemptmaxscore', constants::PENALTY_MAX);
        }
        return true;
    }
