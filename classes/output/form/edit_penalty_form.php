...
namespace gradepenalty_reattemptmaxscore\output\form;
...
use core\url;
use gradepenalty_reattemptmaxscore\constants;
use gradepenalty_reattemptmaxscore\penalty_rule;
use moodleform;

/**
 * Form to set up the penalty rules for the gradepenalty_reattemptmaxscore plugin.
 *
...
class edit_penalty_form extends moodleform {
    ...
    public function definition(): void {
        ...
        if (!is_null($finalpenaltyrule)) {
            ...
            if (!empty($existingrules)) {
                ...
                foreach ($existingrules as $rule) {
                    $repeatedrules[] = [
                        'reattemptby' => $rule->get('reattemptby'),
        ...
        if (!is_null($finalpenaltyrule)) {
            ...
            foreach ($repeatedrules as $rulenumber => $rule) {
                $data['reattemptby[' . $rulenumber . ']'] = $rule['reattemptby'];
    ...
    public function validation($data, $files): array {
        ...
        if (empty($data['reattemptby'])) {
            return $errors;
        }

        // The late for and penalty values must be in ascending order.
        $reattemptbylowerbound = constants::REATTEMPTBY_MIN - 1;
        $reattemptbyupperbound = constants::REATTEMPTBY_MAX + 1;
        $penaltylowerbound = constants::PENALTY_MIN - 1;
        $penaltyupperbound = constants::PENALTY_MAX + 1;

        // Go to each group.
        foreach ($data['reattemptby'] as $rulenumber => $reattemptby) {
            $rulegroupid = 'rulegroup[' . $rulenumber . ']';

            // Skip validation if user did not change default reattempt value. We will remove those rules later.
            if ($reattemptby < constants::REATTEMPTBY_MIN) {
                continue;
            }

            // Validate reattempt field.
            if ($reattemptby <= $reattemptbylowerbound) {
            ...
            if ($penalty >= $penaltyupperbound) {
                if ($penaltylowerbound == constants::PENALTY_MAX) {
                    // Minimum value a penalty can have.
                    $errormessage = get_string('error_penalty_minvalue', 'gradepenalty_reattemptmaxscore',
            ...
            } else if ($penalty <= $penaltylowerbound) {
                // Validate max value of penalty.
                $errors[$rulegroupid] = get_string('error_penalty_maxvalue', 'gradepenalty_reattemptmaxscore',
                    format_float(constants::PENALTY_MIN));
            } else {
                $penaltyupperbound = $penalty;
            }
        }

        // Check the penalty of the final rule. It must be greater than the last rule.
        $finalpenalty = $data['finalpenaltyrule'];
        if ($finalpenalty >= $penaltyupperbound) {
            $errors['finalpenaltyrule'] = get_string('error_penalty_abovevalue', 'gradepenalty_reattemptmaxscore',
                format_float($penaltyupperbound));
        } else if ($finalpenalty >= $penaltyupperbound) {
            $errors['finalpenaltyrule'] = get_string('error_penalty_maxvalue', 'gradepenalty_reattemptmaxscore',
                format_float($penaltyupperbound));
        }
    ...
    public function save_data($data): void {
        ...
        $newdata = [];
        if (isset($data->reattemptby)) {
            foreach ($data->reattemptby as $rulenumber => $reattemptby) {
                // Remove the invalid default rule that we skipped validating.
                if ($reattemptby >= constants::REATTEMPTBY_MIN) {
                    $newdata[$reattemptby] = $data->penalty[$rulenumber];
                }
            }
            // Sort by reattemptby.
            ksort($newdata);
        ...
        $reattemptby = array_keys($newdata);
        ...
        for ($i = 0; $i < $numofrulesinrepeater; $i++) {
            ...
            $rule->set('reattemptby', $reattemptby[$i]);
    ...
    private static function rule_element(MoodleQuickForm $mform): array {
        ...
        $options['reattemptby']['type'] = PARAM_INT;

        // Penalty.
        $elements[] = $mform->createElement('static', '', '',
            html_writer::span(get_string('penalty_label', 'gradepenalty_reattemptmaxscore'), 'ms-4 me-2'));

        // Penalty value element.
        $elements[] = $mform->createElement('text', 'penalty',
            get_string('penalty_label', 'gradepenalty_reattemptmaxscore'), ['size' => 3, 'maxlength' => 3]);
       ...
       // $elements[] = $mform->createElement('static', '', '', html_writer::span('%', 'me-4'));
        ...
        $menu->add(new action_menu_link(
            new url('#'),
            new pix_icon('t/add', ''),
            get_string('insertrule', 'gradepenalty_reattemptmaxscore'),
            false,
            ['class' => 'insertbelow']
        ));
        ...
        return [
            $mform->createElement(
                'group',
                'rulegroup',
                get_string('penaltyrule_group', 'gradepenalty_reattemptmaxscore'),
                $elements,
                [''],
                false
            ),
