...
class view_penalty_rule_action_bar extends action_bar {
    ...
    public function get_template(): string {
        return 'gradepenalty_reattemptmaxscore/view_penalty_rule_action_bar';
    }
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if (penalty_rule::is_overridden($contextid)) {
            ...
            $info = new notification(
                get_string('penaltyrule_overridden', 'gradepenalty_reattemptmaxscore'),
            ...
            if (penalty_rule::is_inherited($contextid)) {
                ...
                $info = new notification(
                    get_string('penaltyrule_inherited', 'gradepenalty_reattemptmaxscore'),
                    notification::NOTIFY_INFO,
                );
                $data['info'] = $info->export_for_template($output);
            } else {
                ...
                $info = new notification(
                    get_string('penaltyrule_not_inherited', 'gradepenalty_reattemptmaxscore'),
                    notification::NOTIFY_INFO,
                );
                $data['info'] = $info->export_for_template($output);
            }
