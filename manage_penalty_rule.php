...
use core\output\notification;
use core\url;
use gradepenalty_reattemptmaxscore\output\edit_penalty_rule_action_bar;
use gradepenalty_reattemptmaxscore\output\form\edit_penalty_form;
use gradepenalty_reattemptmaxscore\output\view_penalty_rule_action_bar;
use gradepenalty_reattemptmaxscore\penalty_rule;
use gradepenalty_reattemptmaxscore\table\penalty_rule_table;
...
if ($context->contextlevel == CONTEXT_SYSTEM) {
    ...
    require_capability('gradepenalty/reattemptmaxscore:manage', $context);
}

$PAGE->set_context($context);
$url = new url('/grade/penalty/reattemptmaxscore/manage_penalty_rule.php', ['contextid' => $contextid]);
$PAGE->set_url($url);
...
if (!$edit) {
    $title = get_string('reattemptmaxscorerule', 'gradepenalty_reattemptmaxscore');
} else {
    $title = get_string('editreattemptmaxscorerule', 'gradepenalty_reattemptmaxscore');
    // Add edit navigation node.
    $PAGE->navbar->add(get_string('editreattemptmaxscorerule', 'gradepenalty_reattemptmaxscore'));
}
...
if ($reset || $deleteeall) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(get_string('resetconfirm', 'gradepenalty_reattemptmaxscore'), $confirmurl, $url);
    echo $OUTPUT->footer();
    die;
...
if ($edit) {
    ...
    $PAGE->requires->js_call_amd('gradepenalty_reattemptmaxscore/edit_penalty_form', 'init');
}
